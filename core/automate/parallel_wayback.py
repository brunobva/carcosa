...
def parallel():
    os.system('rm -rf ' + dir + '/../../docker/data/' + target + '/temp/parallel_wayback.log')
    with open(dir + '/../../docker/data/' + target + '/temp/parallel_wayback.log', 'a') as file:
        for s in sist:
            file.write('python3 ' + dir + '/../tools/scan_wayback.py ' + target + ' ' + s + ' ' + sist[s][0] + ' ' + sist[s][1] + '\n')
    print(f'{Fore.LIGHTYELLOW_EX}[+] STARTING WAYBACK \nThis may take a while...{Style.RESET_ALL}\n')
    try:
        telegramMsg('[+] STARTING WAYBACK ')
    except:
        pass
    os.system('cat ' + dir + '/../../docker/data/' + target + '/temp/parallel_wayback.log | parallel -u')
