...
subdomains = {}
rand = str(uuid.uuid1())
container_name = target + '-' + 'assetfinder' + rand
output = 'assetfinder-' + rand + '.txt'
...
def exec_docker():
    subprocess.check_output(
        'docker run --rm --name ' + container_name + ' -v ' + dir + '/../../docker/data/' + target + '/temp:/data kali-carcosa:1.0 assetfinder -subs-only ' + domain + ' >> ' + dir + '/../../docker/data/' + target + '/temp/' + output + '  || true',
        shell=True)
...
def parse():
    with open(dir + '/../../docker/data/' + target + '/temp/' + output) as file:
        for l in file:
            ...
            subdomains['server.address'] = l.rstrip('\n')
            subdomains['server.domain'] = l.rstrip('\n')
            try:
                subdomains['server.ip'] = socket.gethostbyname(l.rstrip('\n'))
            except:
